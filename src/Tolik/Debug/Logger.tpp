#ifndef TOLIK_LOGGER_TPP
#define TOLIK_LOGGER_TPP

#ifndef TOLIK_LOGGER_HPP
#error __FILE__ should only be included from Logger.hpp
#endif
...
void Logger::LogMessage(const std::string &format, Args&&... args) const
{
  ...
  while (atSignPosition != std::string::npos)
  {
    ...
    if (index < convertedArguments.size())
    {
      ...
      Print(convertedArguments[convertedArguments.size() - 1 - index]);
