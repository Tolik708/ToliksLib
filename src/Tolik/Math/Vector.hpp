#ifndef TOLIK_VECTOR_HPP
#define TOLIK_VECTOR_HPP
...
#include "Tolik/Math/VectorDeclaration.hpp"
#include "Tolik/Math/Constants.hpp"
#include "Tolik/Utilities/HashFunctions.hpp"
...
template<typename Type, std::size_t Size, VectorFlag Group> struct Vector : public Detail::VectorConstants<Type, Size, Group>, public Detail::MinMaxConstants<Type, Size, Group>
{
  ...
  template<typename... Args, typename std::enable_if_t<(sizeof...(Args) <= Size), bool> = true> constexpr Vector(NonScalar, Args... args) : data{ static_cast<Type>(args)... } {}
  ...
  template<typename T = Type> inline constexpr const T &At(std::size_t index) const
   { if(index < Size) throw std::out_of_range("Too big index " + std::to_string(index)); else return static_cast<T>(data[index]); }
  // Performes bounds checking
  ...
  // Performes bounds checking
  template<typename T = Type> inline constexpr T &At(std::size_t index)
  { if(index < Size) throw std::out_of_range("Too big index " + std::to_string(index)); else return static_cast<T>(data[index]); }
  // Performes bounds checking
