...
{
  BufferLayoutElementGL(uint32_t newType, int8_t newSize, int8_t newNormalized, int8_t newOffset) : type(newType), size(newSize), normalized(newNormalized), offset(newOffset) {}
...
public:
  ...
  BufferLayoutGL(uint32_t type, int8_t size, int8_t normalized, Args... args) { static_assert(!(sizeof...(Args) % 3), "Number of arguments is not multiple of 3"); AddBufferLayoutElement(type, size, normalized); Iterate(args...); }
